...
export default function RoomComparisonTable({ 
  ...
  return (
    <div className="space-y-3 pt-1 font-sans select-none">
      {onToggleMixedMode && (
        <div className="flex flex-wrap items-center justify-center gap-3 border-b border-slate-100 dark:border-slate-800 pb-2.5">
          <div className="flex items-center gap-1 p-1 bg-slate-100/90 dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 rounded-full select-none shadow-3xs">
            <button
              type="button"
            ...
            return (
              <button
                ...
                className={`relative flex flex-col justify-between rounded-2xl p-4 text-left transition-all duration-300 transform cursor-pointer border select-none group hover:-translate-y-0.5 active:scale-[0.99] ${
                  isSelected
                    ? 'bg-gradient-to-br from-emerald-50 via-teal-50/50 to-white dark:from-emerald-950/60 dark:via-teal-950/40 dark:to-slate-900 border-2 border-emerald-600 dark:border-emerald-500 shadow-md ring-4 ring-emerald-500/20 scale-[1.01]'
                    : 'bg-white dark:bg-slate-900 border-slate-200/90 dark:border-slate-800 hover:border-emerald-400 dark:hover:border-emerald-500/50 hover:bg-slate-50/70 dark:hover:bg-slate-800 hover:shadow-sm'
                }`}
              >
                {isSelected && (
                ...
                <div className="space-y-2.5 w-full">
                    ...
                    <div className="flex items-center gap-2">
                      <div className={`h-7 w-7 rounded-xl flex items-center justify-center font-bold text-xs font-mono transition-all ${
                        isSelected 
                          ...
                          : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 group-hover:bg-emerald-100 dark:group-hover:bg-emerald-950/80 group-hover:text-emerald-800 dark:group-hover:text-emerald-300'
                      }`}>
                        {r.occupancy}
                      </div>
                  ...
                  <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/70 dark:border-slate-700/70 text-xs flex items-center justify-between">
                ...
                <div className="mt-3.5 pt-2.5 border-t border-slate-200/80 dark:border-slate-800 flex items-center justify-between w-full">
            ...
            return (
              <div 
                ...
                className={`relative flex flex-col justify-between rounded-2xl p-4 text-left transition-all duration-300 border select-none ${
                  count > 0 
                    ? 'bg-gradient-to-br from-emerald-50 via-teal-50/50 to-white dark:from-emerald-950/60 dark:via-teal-950/40 dark:to-slate-900 border-2 border-emerald-600 dark:border-emerald-500 shadow-md ring-4 ring-emerald-500/20' 
              ...
              >
                ...
                <div className="space-y-3 w-full">
                  ...
                  {/* Oda Adedi Sayacı */}
                  <div className="flex items-center justify-between p-2.5 bg-slate-50 dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700">
                    <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Oda Sayısı:</span>
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() => onChangeMixedRoom?.(r.key, Math.max(0, count - 1))}
                        className="h-7 w-7 rounded-xl bg-white dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 active:scale-90 text-slate-700 dark:text-slate-200 font-bold flex items-center justify-center border border-slate-200 dark:border-slate-600 transition-all cursor-pointer shadow-3xs"
                        title="1 Oda Azalt"
                      >
                  ...
                  {count > 0 && (
                    <div className="flex items-center justify-between text-xs px-1 text-slate-600 dark:text-slate-400 font-semibold">
                      <span>Toplam Misafir:</span>
                      <span className="font-bold text-emerald-800 dark:text-emerald-400 font-mono">{subPax} Kişi</span>
                    </div>
                  )}
                </div>

                {/* Alt Dağılım Durumu (Fiyatsız, Sadece Oda & Kişi Durumu) */}
                <div className="mt-3.5 pt-2.5 border-t border-slate-200/80 dark:border-slate-800 flex items-center justify-between text-xs w-full">
                  <span className="text-slate-500 dark:text-slate-400 font-bold">Kapasite:</span>
                  <span className={`font-mono font-black ${
                    count > 0 ? 'text-emerald-800 dark:text-emerald-400' : 'text-slate-400 dark:text-slate-500'
                  }`}>
